...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:
        imgGrayscale, imgThresh = Preprocess.preprocess(possiblePlate.imgPlate)

        try:
            results = reader.readtext(possiblePlate.imgPlate, detail=0, paragraph=False)

            raw_text = "".join(results).upper().replace(" ", "")
            clean_text = "".join(e for e in raw_text if e.isalnum())

            possiblePlate.strChars = clean_text
            print(f"EasyOCR Detected: {possiblePlate.strChars}")

        except Exception as e:
            print(f"Lỗi khi chạy EasyOCR: {e}")
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    listOfMatchingChars.sort(key=lambda c: (c.intCenterY, c.intCenterX))
